...
In order to configure Sonarcloud analysis [GitHub Action workflow](git@github.com:dianna-ai/distance_explainer/blob/main/.github/workflows/sonarcloud.yml) you must follow the steps below:
...
1. go to [new code definition administration page](https://sonarcloud.io/project/new_code?id=dianna-ai_distance_explainer) and select `Number of days` option
1. To be able to run the analysis:
   1. a token must be created at [Sonarcloud account](https://sonarcloud.io/account/security/)
   1. the created token must be added as `SONAR_TOKEN` to [secrets on GitHub](https://github.com/dianna-ai/distance_explainer/settings/secrets/actions)
